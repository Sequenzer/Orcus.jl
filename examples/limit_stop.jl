...
A = Asset("AAPL", ohlc((100.0,102.0,99.0,101.0), (100.0,101.0,99.0,100.0), (96.0,97.0,95.0,96.0)),
    ["Open","High","Low","Close"])
...
println("bar3 (Low=95): filled? ", isfulfilled(O), "  fill price: ", first(values(B.portfolio)).avg_cost)

println("\n== 2. Stop order: breakout entry ==")
A2 = Asset("TSLA", ohlc((100.0,102.0,99.0,101.0), (100.0,101.0,99.0,100.0), (100.0,106.0,99.0,104.0)),
    ["Open","High","Low","Close"])
...
advance_to!(M2,3); process_orders!(B2)
println("bar3 (High=106): triggered? ", isfulfilled(O2), "  fill price: ", first(values(B2.portfolio)).avg_cost)

println("\n== 3. Gap-through: fill at the better open, not the stated price ==")
A3 = Asset("NFLX", ohlc((100.0,102.0,99.0,101.0), (90.0,92.0,88.0,90.0)), ["Open","High","Low","Close"])
...
println("limit was 95, open gapped to 90 -> filled at: ", first(values(B3.portfolio)).avg_cost)
...
B4 = Broker(M4, 500.0)
p = price(Buy(A4))
println("unit price ~", round(p,digits=2), " -> 100 units would cost ~", round(100p,digits=2), ", cash is 500")
O4 = Order(Buy(A4), 100; allow_partial=true)   # opt-in
place_order!(B4,O4)
process_orders!(B4)
println("filled this bar: ", B4.history[1].volume, " units, cash left: ", round(B4.cash,digits=4))
println("still resting: ", remaining(O4), " units, order still in book? ", !isempty(B4.orders))
