...
println("loan: ", P.loan)
println("equity right after entry: ",
  B.cash + total_value(B.portfolio) - total_loan(B.portfolio),
...
println(
  "P&L: ",
  B.cash - 5000.0,
  " on a \$5000 stake -- the 2x leverage doubled the 20% move to 40%",
)

println("\n== 3. Shorts: unconstrained under NoMargin, gated under RegTMargin ==")
A2 = Asset(
  "TSLA", ohlc(fill((200.0, 200.0, 200.0, 200.0), 2)...), ["Open", "High", "Low", "Close"]
)
M2 = Market([A2]);
advance_to!(M2, 1)
...
A3 = Asset(
  "MSFT", ohlc(fill((100.0, 100.0, 100.0, 100.0), 3)...), ["Open", "High", "Low", "Close"]
)
M3 = Market([A3]);
advance_to!(M3, 1)
...
process_all!(B3)
println(
  "cash after 2 bars of static price: ", B3.cash, " (", cash0, " minus 2 * 5000*0.001)"
)

println("\n== 5. Liquidation on a maintenance breach ==")
A4 = Asset(
  "GME",
  ohlc((100.0, 100.0, 100.0, 100.0), (50.0, 50.0, 50.0, 50.0)),
  ["Open", "High", "Low", "Close"],
)
M4 = Market([A4]);
advance_to!(M4, 1)
...
process_orders!(B4)
advance_to!(M4, 2);
process_all!(B4)     # price halves to 50: equity = 0+5000-5000 = 0 < req(1250)
