...
@testset verbose=false "Broker" begin
  ...
  @testset verbose=false "limit/stop orders" begin
    ...
    data = ohlc(
      (100.0, 102.0, 99.0, 101.0), (100.0, 101.0, 99.0, 100.0), (96.0, 97.0, 95.0, 96.0)
    )
    ...
    data4 = ohlc(
      (100.0, 102.0, 99.0, 101.0), (100.0, 104.0, 99.0, 102.0), (106.0, 108.0, 105.0, 107.0)
    )
    ...
    A5 = asset()
    @test_throws MethodError check_trigger(
      limit_order(LongCall(A5, 10), 1, 50.0), broker(1, 1000), 1
    )
  end
  ...
  @testset verbose=false "margin / leverage / liquidation" begin
    ...
    A5 = Asset(
      "M5", ohlc(fill((100.0, 100.0, 100.0, 100.0), 3)...), ["Open", "High", "Low", "Close"]
    )
    M5 = Market([A5]);
    advance_to!(M5, 1)
    ...
    A6 = Asset("M6",
      ohlc(
        (100.0, 100.0, 100.0, 100.0), (80.0, 80.0, 80.0, 80.0), (50.0, 50.0, 50.0, 50.0)
      ),
      ["Open", "High", "Low", "Close"])
    M6 = Market([A6]);
    advance_to!(M6, 1)
    ...
    advance_to!(M6, 2);
    process_all!(B6)          # price 80: equity=0+8000-5000=3000 >= req(2000)
    @test isempty(B6.margin_calls)
    @test length(B6.portfolio) == 1

    advance_to!(M6, 3);
    process_all!(B6)          # price 50: equity=0+5000-5000=0 < req(1250)
